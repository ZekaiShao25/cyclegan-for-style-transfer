...
def get_scheduler(optimizer, opt):
    ...
    if opt.lr_policy == 'linear':
        ...
        scheduler = lr_scheduler.StepLR(optimizer, step_size=opt.lr_decay_iters, gamma=0.98)
...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[], weights_required=True):
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netD == 'n_layers_L':  # more options
        net = NLayerDiscriminator_L(input_nc, ndf, n_layers_D, norm_layer=norm_layer)
...
class ConvNeXtGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, n_blocks=12):
        """Construct a Resnet-based generator
        ...
        mult = 2 ** n_downsampling
        for i in range(n_blocks):       # add ConvNeXt blocks

            model += [ConvXBlock(ngf * mult)]
...
class NLayerDiscriminator_L(nn.Module):
    """Defines a PatchGAN discriminator"""
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        Parameters:
            ...
            norm_layer      -- normalization layer
        """
        super(NLayerDiscriminator_L, self).__init__()
        if type(norm_layer) == functools.partial:  # no need to use bias as BatchNorm2d has affine parameters
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
...
class Dis(nn.Module):
    ...
    """
    def __init__(self,in_channels,negative_slope = 0.2, outchannels = 1):
        ...
        self.conv5 = nn.Conv2d(in_channels=512,out_channels=outchannels,kernel_size=4,stride=2,padding=2)
